...
# Making a function that applies weight decay to solutions produced by ES optimizers.
# This is done to faster optimize the model.
...
class SimpleGA:
    ...
    def __init__(
        ...
        self.elite_params = np.zeros((self.elite_popsize,
                                      self.num_params))
    ...
    def tell(self, reward_table_result):
        ...
        if (not self.forget_best or self.first_iteration):
            ...
            reward = np.concatenate([reward_table,
                                     self.elite_rewards])
            solution = np.concatenate([self.solutions,
                                       self.elite_params])
    ...
    def ask(self):
        if self.antithetic:
            self.epsilon_half = np.random.randn(self.half_popsize,
                                                self.num_params)
            self.epsilon = np.concatenate([self.epsilon_half,
                                           - self.epsilon_half])
        else:
