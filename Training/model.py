...
class Model:
    ...
    # Making a method that creates an environment (in our case the CarRacing game)
    # inside which both the AI and HI will play
    def make_env(
            self,
            seed=-1,
            render_mode=False,
    ...
    def set_model_params(
        ...
        if EXP_MODE == MODE_Z_HIDDEN:
            ...
            self.weight_hidden = params_1[self.hidden_size:].reshape(self.input_size,
                                                                     self.hidden_size)
...
def simulate(
    ...
    for episode in range(num_episode):
        ...
        recording_logvar = np.array(recording_logvar,
                                    dtype=np.float16)
