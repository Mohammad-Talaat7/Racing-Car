...
# Making a function that prints the training information in the standard output
# (this function is created because we are using parallel execution for each worker)
...
# Making a function that encodes and packs solutions from the workers into packets
# (a packet is a set of data sent as one element, to fit the format of the MPI framework)


def encode_solution_packets(
        seeds,
        solutions,
        train_mode=1,
        max_len=-1
...
# Making a function that creates a worker with a specific set of weights (parameters) for the model,
# runs the simulation with them, and returns the average reward after the simulation is played
...
# Making a function that creates a slave
# (a slave is an instance of the model with specific solutions and weights)for the main model,
# gets the possible solutions of the environment, creates a worker for each solution, and returns the reward
...
# Making a function that sends training data to each slave
...
# Making a function that receives packets from slaves after the slaves are done with the simulation process
# and gathered all the information (e.g. the average reward)


def receive_packets_from_slaves():
    result_packet = np.empty(
        RESULT_PACKET_SIZE,
        dtype=np.int32
    )
    reward_list_total = np.zeros((population, 2))
    check_results = np.ones(
        population,
        dtype=np.int
    )
...
# Making a function that evaluates a batch of states/frames with the provided model parameters
...
# Making a function that implements the whole training process
...
def master():
    ...
    while True:
        ...
        h = (
            ...
            r_max,
            std_reward,
            int(es.rms_stdev()*100000)/100000.,
            mean_time_step+1.,
            int(max_time_step)+1
        )
        if cap_time_mode:
...
# Making a function that executes the training
...
def main(args):
    ...
    initialize_settings(args.sigma_init,
                        args.sigma_decay)
    sprint(
        "process",
        rank,
...
# Implementing the main code that launches the whole training with a chosen set of parameters
