...
class MDNRNN(object):
    ...
    def build_model(
        ...
        self.input_x = tf.placeholder(
            dtype=tf.float32,
            shape=[self.hps.batch_size, self.hps.max_seq_len, INWIDTH]
        )
        self.output_x = tf.placeholder(
            dtype=tf.float32,
            shape=[self.hps.batch_size, self.hps.max_seq_len, OUTWIDTH]
        )
        ...
        def get_mdn_coef(output):
            ...
            logmix = logmix - tf.reduce_logsumexp(
                logmix,
                1,
                keepdims=True
            )
        ...
        def get_lossfunc(
            ...
            v = tf.reduce_logsumexp(
                v,
                1,
                keepdims=True
            )
            return -tf.reduce_mean(v)
        flat_target_data = tf.reshape(
            self.output_x,
            [-1, 1]
        )
    ...
    def load_json(
            self,
            jsonfile='Weights/rnn_weights.json'
    ):
...
def sample_sequence(
    ...
    for i in range(seq_len):
        input_x = np.concatenate(
            (prev_x, actions[i].reshape((1, 1, 3))), axis=2)
...
def rnn_next_state(
    ...
    input_x = np.concatenate(
        (z.reshape((1, 1, 32)), a.reshape((1, 1, 3))), axis=2)
